...
describe('BoardController', () => {
  ...
  it('findAll should pass user id', async () => {
    await controller.findAll(mockReq);
    expect(service.findAll).toHaveBeenCalledWith('user-1');
  });

  it('findOne should pass board id', async () => {
    await controller.findOne('board-1');
    expect(service.findOne).toHaveBeenCalledWith('board-1');
  });

  it('update should pass board id and dto', async () => {
    await controller.update('board-1', { name: 'Updated' });
    expect(service.update).toHaveBeenCalledWith('board-1', { name: 'Updated' });
  });

  it('remove should pass board id', async () => {
    await controller.remove('board-1');
    expect(service.remove).toHaveBeenCalledWith('board-1');
  });

  it('invite should pass board id and target user id', async () => {
    await controller.invite('board-1', 'user-2');
    expect(service.invite).toHaveBeenCalledWith('board-1', 'user-2');
  });

  it('kick should pass board id and target user id', async () => {
    await controller.kick('board-1', 'user-2');
    expect(service.kick).toHaveBeenCalledWith('board-1', 'user-2');
  });
});
